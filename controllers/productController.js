...
//file system module, which provides functions
...
// payment processing library for integrating payment gateways
import braintree from "braintree";

// used to load environment variables 
//often used to store sensitive information 
//like API keys and secrets.
import dotenv from "dotenv";

// invokes the config method 
//allows to access environment variables in your code
dotenv.config();
...
//used as a controller for creating new products.
export const createProductController = async (req, res) => {
  try {
    ...
    //validation
    //it will execute the case blocks where the 
...
export const getProductController = async (req, res) => {
  try {
    const products = await productModel
      .find({}) // fetches all products without any specific conditions.
      .populate("category") // to populate a field in the retrieved documents
      .select("-photo")

      .limit(12) //limits the number of products retrieved to 12

      .sort({ createdAt: -1 }); // sorts the products

    res.status(200).send({ //code sets the HTTP response
      success: true,
